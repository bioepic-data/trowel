...
# 2. Generate embeddings (using CurateGPT)
curategpt -vvv index \
  -c bervo \
  -D duckdb \
  --batch-size 100 \
...
```bash
# 1. Prepare and embed both ontologies
trowel embeddings prepare-embeddings \
  -i ontology1.csv \
...
# 2. Generate embeddings (using CurateGPT for each)
...
- [EMBEDDING_QUICK_START.md](EMBEDDING_QUICK_START.md) - Quick reference guide
- [EMBEDDING_CLUSTERING.md](EMBEDDING_CLUSTERING.md) - Comprehensive embedding command documentation
- [EMBEDDING_COMMANDS_INDEX.md](EMBEDDING_COMMANDS_INDEX.md) - Command index and navigation
- [NOTEBOOK_TO_CLI_MAPPING.md](NOTEBOOK_TO_CLI_MAPPING.md) - How notebook functions map to CLI commands
